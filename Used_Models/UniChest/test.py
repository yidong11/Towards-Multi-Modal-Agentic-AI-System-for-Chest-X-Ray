...
from dataset.test_dataset import Chestxray14_Dataset,CheXpert_Dataset,Padchest_Dataset,Vindr_Dataset, SIIMACR_Dataset, Shenzhen_Dataset, Openi_Dataset, MIMIC_Test_Dataset
...
def main(args, config):
    ...
    elif args.test_data == 'mimic':
        test_dataset = MIMIC_Test_Dataset(config['mimic_test_file'],config['mimic_label_file'],config['image_res'])
        # MIMIC_Dataset(config['train_entity_file'], config['train_fg_query_file_v1'], config['mrsty_file'],config['image_res'], args)
        test_dataloader =DataLoader(
                test_dataset,
                batch_size=config['batch_size'],
                num_workers=4,
    ...
    if args.bert_model_name == 'emilyalsentzer/Bio_ClinicalBERT':
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.bert_model_name,do_lower_case=True, local_files_only=False)
        text_encoder = CLP_clinical(bert_model_name=args.bert_model_name).cuda()

    if args.bert_pretrained:
        checkpoint = torch.load(args.bert_pretrained, map_location='cpu', weights_only=False)
    ...
    if args.test_data == 'chexpert':
        ...
        save_result_path = os.path.join(args.save_result_dir,'mimic')
        Path(save_result_path).mkdir(parents=True, exist_ok=True)

        text_list = ['Atelectasis','Cardiomegaly', 
                        'Consolidation', 'Edema', 'Enlarged Cardiomediastinum', 'Fracture', 'Lung Lesion',
                        'Lung Opacity', 'No Finding','Pleural Effusion', 'Pleural Other', 'Pneumonia', 
                        'Pneumothorax', 'Support Devices']

        dist_csv_col =['Metric', 'Atelectasis','Cardiomegaly', 
                        'Consolidation', 'Edema', 'Enlarged Cardiomediastinum', 'Fracture', 'Lung Lesion',
                        'Lung Opacity', 'No Finding','Pleural Effusion', 'Pleural Other', 'Pneumonia', 
                        'Pneumothorax', 'Support Devices']
        test(model,image_encoder, text_encoder, tokenizer, test_dataloader,device,save_result_path,args,text_list,dist_csv_col)
