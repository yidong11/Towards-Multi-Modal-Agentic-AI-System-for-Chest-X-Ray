...
def test(model,image_encoder, text_encoder, tokenizer, data_loader,device,save_result_path,args,text_list,dist_csv_col):
    ...
    for i, sample in tqdm(enumerate(data_loader)):
        ...
        subject_id_list.extend(subject_id)  # batch size of 32
        study_id_list.extend(study_id)  # batch size of 32
    ...
    df = pd.DataFrame(columns=["subject_id", "study_id"] + text_list)
    df["subject_id"] = subject_id_list
    df["study_id"] = study_id_list
    df.iloc[:, 2:] = np.round(array_pred, 3)
    # Convert each row into a stringified dict with 3-decimal rounded floats
    df['classification_result'] = df.apply(lambda row: 
        str([(col, round(row[col], 3)) for col in df.columns[2:]]), axis=1)

    # Optional: drop the original float columns if only 'combined' is needed
    df = df[['subject_id', 'study_id', 'classification_result']]
    print(df.head(), len(study_id_list), len(subject_id_list), array_pred.shape)
    df.to_csv(os.path.join(save_result_path, "unichest-train-2k.csv"), index=False)
